...
def extend_maze(maze):
    original_array = maze
    # Новый массив размером 32x32x2
    new_shape = (31, 31, 2)
    new_array = np.zeros(new_shape)
    # Заполнение нового массива
    new_array[::2, ::2] = original_array  # Копируем исходные точки
    # Добавляем промежуточные точки по x
    new_array[::2, 1::2] = (original_array[:, :-1, :] + original_array[:, 1:, :]) / 2
    # Добавляем промежуточные точки по y
    new_array[1::2, ::2] = (original_array[:-1, :, :] + original_array[1:, :, :]) / 2
    # Добавляем промежуточные точки по диагонали
    new_array[1::2, 1::2] = (original_array[:-1, :-1, :] + original_array[1:, 1:, :]) / 2
    return new_array
...
class CarController(Robot):
    def __init__(self, kp_linear=1, kd_linear=1, kp_angular=1, kd_angular=1, distance_offset=20.0, angle_offset = 0.2, success_rate = 1,
        ...
        self.maze = extend_maze(get_maze())
        self.position = np.array((30,0))
    ...
    def _shift_move(self, shift):
        ...
        # TODO: проверка на флоат шифт. 
        # TODO: определение текущего направления
        # TODO: определение смещения до цели
        # TODO: контроль этого смещения
        # TODO: определение новой позиции
    ...
    def _shift_rotate(self, shift):
        ...
        # self._current_orientation = self.orientation_dict[target_angle] # TODO
    ...
    def _nearest_angle(self, angle):
        ...
        target_angles = [0, 45, 90, 135, 180, -45, -90, -135]
