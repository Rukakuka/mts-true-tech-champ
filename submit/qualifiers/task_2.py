...
class ApiClient:
    def __init__(
        self, api_token: str, max_retries: int, timeout: float, sleep_time: float
    ) -> None:
        self._api_token = api_token
        self._url_sensors = (
            f"http://127.0.0.1:8801/api/v1/robot-cells/sensor-data?token={api_token}"
        )
        self._url_forward = (
            f"http://127.0.0.1:8801/api/v1/robot-cells/forward?token={api_token}"
        )
        self._url_backward = (
            f"http://127.0.0.1:8801/api/v1/robot-cells/backward?token={api_token}"
        )
        self._url_left = (
            f"http://127.0.0.1:8801/api/v1/robot-cells/left?token={api_token}"
        )
        self._url_right = (
            f"http://127.0.0.1:8801/api/v1/robot-cells/right?token={api_token}"
        )
    ...
    def read_sensors(self) -> SensorsReading:
        response = self._request(request_type="get", url=self._url_sensors)
    ...
    def reset_maze(self):
        ...
        self._request(request_type="post", url=url)

    def send_maze(self, maze: np.ndarray) -> int:
        url = f"http://127.0.0.1:8801/api/v1/matrix/send?token={self._api_token}"
        response = self._request(request_type="post", url=url, data=maze.tolist())
    ...
    def _request(
        self, request_type: str, url: str, data: dict = None
    ) -> requests.Response:
...
class FeedbackRobotController(AbstractRobotController):
    def __init__(
        self,
        sensors_reader: SimpleSensorReader,
        api_client: ApiClient,
        delay: Optional[float],
        api_move_wait_timeout: float = 5.0,
        api_command_retries: int = 1,
    ):
